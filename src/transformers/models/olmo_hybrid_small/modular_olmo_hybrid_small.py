...
"""
OlmoHybridSmall: A hybrid model with GatedDeltaNet + gated NoPE attention,
...
@strict
class OlmoHybridSmallConfig(OlmoHybridConfig):
    r"""
...
class OlmoHybridSmallRMSNorm(OlmoHybridRMSNorm):
    pass


class OlmoHybridSmallRMSNormGated(OlmoHybridRMSNormGated):
    pass


class OlmoHybridSmallMLP(OlmoHybridMLP):
    pass


class OlmoHybridSmallDynamicCache(OlmoHybridDynamicCache):
    pass


class OlmoHybridSmallGatedDeltaNet(OlmoHybridGatedDeltaNet):
    pass


class OlmoHybridSmallAttention(OlmoHybridAttention):
    """
    Full attention for OlmoHybridSmall with:
    ...
    def __init__(self, config: OlmoHybridSmallConfig, layer_idx: int):
        ...
        if config.use_head_qk_norm:
            self.q_norm = OlmoHybridSmallRMSNorm(self.head_dim, config.rms_norm_eps)
            self.k_norm = OlmoHybridSmallRMSNorm(self.head_dim, config.rms_norm_eps)
...
class OlmoHybridSmallAttentionDecoderLayer(OlmoHybridAttentionDecoderLayer):
    """
    ...
    OlmoHybridSmall uses peri_norm (pre+post):
        residual + post_norm(attn(pre_norm(x)))
    """

    def __init__(self, config: OlmoHybridSmallConfig, layer_idx: int):
        super().__init__(config, layer_idx)
        self.self_attn = OlmoHybridSmallAttention(config=config, layer_idx=layer_idx)
        # peri_norm: pre-norm (input_layernorm) + post-norm (post_attention_layernorm)
        self.input_layernorm = OlmoHybridSmallRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = OlmoHybridSmallRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        # peri_norm for FFN: pre-norm (ffn_layernorm) + post-norm (post_feedforward_layernorm)
        self.ffn_layernorm = OlmoHybridSmallRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_feedforward_layernorm = OlmoHybridSmallRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.mlp = OlmoHybridSmallMLP(config)
...
class OlmoHybridSmallLinearAttentionDecoderLayer(OlmoHybridLinearAttentionDecoderLayer):
    """
    ...
    def __init__(self, config: OlmoHybridSmallConfig, layer_idx: int):
        super().__init__(config, layer_idx)
        self.linear_attn = OlmoHybridSmallGatedDeltaNet(config, layer_idx=layer_idx)
        self.input_layernorm = OlmoHybridSmallRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = OlmoHybridSmallRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.ffn_layernorm = OlmoHybridSmallRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_feedforward_layernorm = OlmoHybridSmallRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
class OlmoHybridSmallPreTrainedModel(OlmoHybridPreTrainedModel):
    _no_split_modules = ["OlmoHybridSmallAttentionDecoderLayer", "OlmoHybridSmallLinearAttentionDecoderLayer"]
    _can_record_outputs = {
        "hidden_states": (OlmoHybridSmallAttentionDecoderLayer, OlmoHybridSmallLinearAttentionDecoderLayer),
        "attentions": OlmoHybridSmallAttention,
    }

    @torch.no_grad()
    def _init_weights(self, module):
        PreTrainedModel._init_weights(self, module)
        if isinstance(module, OlmoHybridSmallGatedDeltaNet):
...
class OlmoHybridSmallModel(OlmoHybridModel):
    def __init__(self, config: OlmoHybridSmallConfig):
        super().__init__(config)
        # Replace layers with peri-norm variants
        self.layers = nn.ModuleList(
            [
                OlmoHybridSmallLinearAttentionDecoderLayer(config, layer_idx)
                if config.layer_types[layer_idx] == "linear_attention"
                else OlmoHybridSmallAttentionDecoderLayer(config, layer_idx)
                for layer_idx in range(config.num_hidden_layers)
            ]
        )
        # Embedding norm (applied after embed_scale)
        self.embed_norm = OlmoHybridSmallRMSNorm(config.hidden_size, eps=config.embedding_norm_eps)
        # Final norm before LM head
        self.norm = OlmoHybridSmallRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if use_cache and past_key_values is None:
            past_key_values = OlmoHybridSmallDynamicCache(config=self.config)
...
class OlmoHybridSmallForCausalLM(OlmoHybridForCausalLM):
    pass


__all__ = [
    "OlmoHybridSmallConfig",
    "OlmoHybridSmallForCausalLM",
    "OlmoHybridSmallModel",
    "OlmoHybridSmallPreTrainedModel",
]
